...
heading2 tc =
    let
        parsed_ =
            case Parser.parseHeading tc.generation tc.count tc.source of
                Ok goodstuff ->
                    [ goodstuff ]
...
item tc =
    let
        parsed_ =
            case Parser.parseItem tc.generation tc.count tc.source of
                Ok goodstuff ->
                    [ goodstuff ]
...
pipe tc =
    ...
    in
    { tc | complete = Parser.parse tc.generation tc.count source_ :: tc.complete }
